function PlotVelRMSE(TTs, NEV, vis_sats, used_sats, range)
    %   in  :   TTs:    gs time tag
    %           NEV:    dNEV 
    ...
    title(sprintf('Circle Rad : %.2fm/s, %.2fm/s',round2,round1));
    ...
    xlabel('Hours'); ylabel('Vertical (m/s)');
    title(sprintf('RMSE : %.4fm/s', rmse_v));
    % ylim([-10 10]);

    %% 6. 가운데 하단: Horizontal 오차
    axes('Position',[midX, botY, midW, midH]);
    plot(t, horizontal, 'k'); grid on;
    xlabel('Hours'); ylabel('Horizontal (m/s)');
    title(sprintf('RMSE : %.4fm/s', rmse_h));
    % ylim([0 7]);
    ...
    title(sprintf('RMSE : %.4fm/s', rmse_3d));
    ...
    % 데이터 텍스트
    yData = (1-rowGap)/2;
    values = {sprintf('%.2fm/s',rmse_v), sprintf('%.2fm/s',rmse_h), sprintf('%.2fm/s',rmse_3d)};
    for i = 1:3
        x = (i-0.5)/3;
